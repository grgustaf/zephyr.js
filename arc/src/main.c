        ...
        if (msg->bus < MAX_I2C_BUS) {
            ...
            if (!i2c_device[msg->bus]) {
                ...
                if (i2c_configure(i2c_device[msg->bus], cfg.raw) != 0) {
                    ...
                    //Skip the pin sleep section as it slows I2C transmission greatly
                    dont_sleep = true;
...
ipm_send_msg(MSG_ID_I2C, msg->block, reply_type, 0, 0);
}
...
void ipm_msg_receive_callback(void *context, uint32_t id, volatile void *data)
{
    ...
    if (id == MSG_ID_AIO) {
        ...
        if (i2cMsg) {
            //Currently we should be blocked until the current I2C message has been handled
            PRINT("ARC - current i2c msg hasn't been sent yet, not ready for another\n");
        } else {
            //Save the the message pointer to handle it once the main loop comes around
